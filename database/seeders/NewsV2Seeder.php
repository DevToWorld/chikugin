...
class NewsV2Seeder extends Seeder
{
    public function run(): void
    {
        ...
        // Check which table structure we're dealing with
        ...
        $hasDescription = Schema::hasColumn('news', 'description');
        $hasCategory = Schema::hasColumn('news', 'category');
        $hasType = Schema::hasColumn('news', 'type');
        ...
        foreach ($baseItems as $i => $item) {
            ...
            // Add fields based on which table structure exists
            if ($hasDescription) {
                $row['description'] = $item['excerpt'];
            }
            if ($hasCategory) {
            ...
            // Skip membership_requirement as it has a default value and causes issues
            // if ($hasMembershipRequirement) {
            //     $row['membership_requirement'] = 'none';
            // }
            // Always set created_by and updated_by since they are required
            $adminId = DB::table('admins')->value('id') ?? 1; // Fallback to 1 if no admin exists
